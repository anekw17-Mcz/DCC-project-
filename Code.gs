...
function _destroyChart(id) {
  if (_dc[id]) { try { _dc[id].destroy(); } catch(e){} delete _dc[id]; }
}
function _monthVal(m) {
  var mo = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  var p = (m||'').split('-');
  return p.length === 2 ? parseInt('20'+p[1])*100 + mo.indexOf(p[0]) : 0;
}
function _initDashFilters() {
  if (!reportData || !reportData.checked || !reportData.checked.dccData) return;
  var data = reportData.checked.dccData;
  var monthSel = document.getElementById('dash-month-filter');
  var deptSel  = document.getElementById('dash-dept-filter');
  if (!monthSel || !deptSel) return;
  var prevMonth = monthSel.value, prevDept = deptSel.value;
  var months = [], seen = {};
  data.forEach(function(r){ var m=(r[1]||'').trim(); if(m&&!seen[m]){ seen[m]=1; months.push(m); }});
  months.sort(function(a,b){ return _monthVal(b)-_monthVal(a); });
  monthSel.innerHTML = '<option value="">ทุกเดือน</option>' +
    months.map(function(m){ return '<option value="'+m+'"'+(m===prevMonth?' selected':'')+'>'+m+'</option>'; }).join('');
  if (!prevMonth) {
    var cur = new Date().toLocaleDateString('en-GB',{month:'short',year:'2-digit'}).replace(' ','-');
    if (months.indexOf(cur) >= 0) monthSel.value = cur;
  }
  var depts = [], dseen = {};
  data.filter(function(r){ return (r[5]||'')==='Checklist'; }).forEach(function(r){
    var d=(r[6]||'').trim(); if(d&&!dseen[d]){ dseen[d]=1; depts.push(d); }
  });
  depts.sort();
  deptSel.innerHTML = '<option value="">ทุกแผนก</option>' +
    depts.map(function(d){ return '<option value="'+d+'"'+(d===prevDept?' selected':'')+'>'+d+'</option>'; }).join('');
}

function runSummary() {
  if (!reportData || !reportData.checked || !reportData.checked.dccData) {
    document.getElementById('summary-empty').style.display = 'block'; return;
  }
  document.getElementById('summary-empty').style.display = 'none';
  _initDashFilters();
  var selMonth = (document.getElementById('dash-month-filter')||{}).value || '';
  var selDept  = (document.getElementById('dash-dept-filter')||{}).value  || '';
  var all = reportData.checked.dccData;
  var cl = all.filter(function(r){ return (r[5]||'')==='Checklist'; });
  if (selMonth) cl = cl.filter(function(r){ return (r[1]||'').trim()===selMonth; });
  if (selDept)  cl = cl.filter(function(r){ return (r[6]||'').trim()===selDept;  });
  var mo = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  var areaData = reportData.areaData || [];
  if (selMonth) {
    var mIdx = mo.indexOf((selMonth.split('-')[0]||''));
    var mNum = mIdx >= 0 ? String(mIdx+1).padStart(2,'0') : '';
    if (mNum) areaData = areaData.filter(function(r){ return String(r[0]).includes('/'+mNum+'/'); });
  }
  var totalCheck = cl.reduce(function(s,r){ return s+(parseFloat(r[10])||0); }, 0);
  var totalError = cl.reduce(function(s,r){ return s+(parseFloat(r[11])||0); }, 0);
  var errRate = totalCheck > 0 ? (totalError/totalCheck*100).toFixed(1) : '0.0';
  document.getElementById('kpi-total-check').textContent = totalCheck.toLocaleString();
  document.getElementById('kpi-total-error').textContent = totalError.toLocaleString();
  document.getElementById('kpi-error-rate').textContent  = errRate + '%';
  document.getElementById('kpi-area-issues').textContent = areaData.length.toLocaleString();
  _chartDept(cl); _chartTrend(all, selDept); _chartSubfunc(cl); _chartArea(areaData);
}
function _chartDept(cl) {
  _destroyChart('dept');
  var map = {};
  cl.forEach(function(r){ var d=(r[6]||'').trim(),e=parseFloat(r[11])||0; if(d&&e>0) map[d]=(map[d]||0)+e; });
  var items = Object.entries(map).sort(function(a,b){return b[1]-a[1];}).slice(0,8);
  if (!items.length) return;
  _dc.dept = new Chart(document.getElementById('chart-dept').getContext('2d'), {
    type:'bar',
    data:{ labels:items.map(function(x){return x[0];}),
      datasets:[{label:'Error',data:items.map(function(x){return x[1];}),
        backgroundColor:items.map(function(_,i){return 'hsla('+(220-i*18)+',75%,'+(55+i*2)+'%,0.85)';}),
        borderRadius:6,borderSkipped:false}]},
    options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false},tooltip:{callbacks:{label:function(c){return ' '+c.raw.toLocaleString()+' errors';}}}},
      scales:{x:{grid:{color:'rgba(0,0,0,0.04)'},ticks:{font:{size:11}}},y:{grid:{display:false},ticks:{font:{size:11}}}}}
  });
}
function _chartTrend(all, selDept) {
  _destroyChart('trend');
  var now = new Date(), months6 = [];
  for (var i=5;i>=0;i--) {
    var d = new Date(now.getFullYear(), now.getMonth()-i, 1);
    months6.push(d.toLocaleDateString('en-GB',{month:'short',year:'2-digit'}).replace(' ','-'));
  }
  var cl = all.filter(function(r){return (r[5]||'')==='Checklist';});
  if (selDept) cl = cl.filter(function(r){return (r[6]||'').trim()===selDept;});
  var errMap={}, unitMap={};
  cl.forEach(function(r){ var m=(r[1]||'').trim();
    errMap[m]=(errMap[m]||0)+(parseFloat(r[11])||0);
    unitMap[m]=(unitMap[m]||0)+(parseFloat(r[10])||0);
  });
  _dc.trend = new Chart(document.getElementById('chart-trend').getContext('2d'), {
    type:'line',
    data:{labels:months6,datasets:[
      {label:'Error',data:months6.map(function(m){return errMap[m]||0;}),
        borderColor:'#ef4444',backgroundColor:'rgba(239,68,68,0.12)',fill:true,tension:0.4,pointBackgroundColor:'#ef4444',pointRadius:4},
      {label:'ตรวจ',data:months6.map(function(m){return unitMap[m]||0;}),
        borderColor:'#3b82f6',backgroundColor:'rgba(59,130,246,0.07)',fill:true,tension:0.4,pointBackgroundColor:'#3b82f6',pointRadius:4}
    ]},
    options:{responsive:true,maintainAspectRatio:false,
      plugins:{legend:{position:'top',labels:{font:{size:11},boxWidth:12}}},
      scales:{x:{grid:{color:'rgba(0,0,0,0.04)'},ticks:{font:{size:11}}},y:{grid:{color:'rgba(0,0,0,0.04)'},ticks:{font:{size:11}}}}}
  });
}
function _chartSubfunc(cl) {
  _destroyChart('subfunc');
  var map={};
  cl.forEach(function(r){ var s=(r[8]||'').trim(),e=parseFloat(r[11])||0; if(s&&e>0) map[s]=(map[s]||0)+e; });
  var items=Object.entries(map).sort(function(a,b){return b[1]-a[1];}).slice(0,8);
  if(!items.length) return;
  _dc.subfunc = new Chart(document.getElementById('chart-subfunc').getContext('2d'),{
    type:'bar',
    data:{labels:items.map(function(x){return x[0];}),
      datasets:[{label:'Error',data:items.map(function(x){return x[1];}),
        backgroundColor:items.map(function(_,i){return 'hsla('+(25+i*12)+',85%,'+(55+i*2)+'%,0.85)';}),
        borderRadius:6,borderSkipped:false}]},
    options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false},tooltip:{callbacks:{label:function(c){return ' '+c.raw.toLocaleString()+' errors';}}}},
      scales:{x:{grid:{color:'rgba(0,0,0,0.04)'},ticks:{font:{size:10}}},
        y:{grid:{display:false},ticks:{font:{size:10},callback:function(v){var l=this.getLabelForValue(v);return l.length>30?l.slice(0,27)+'…':l;}}}}}
  });
}
function _chartArea(areaData) {
  _destroyChart('area');
  var map={};
  areaData.forEach(function(r){ var a=(r[1]||'').trim(); if(a) map[a]=(map[a]||0)+1; });
  var items=Object.entries(map).sort(function(a,b){return b[1]-a[1];}).slice(0,8);
  if(!items.length) return;
  _dc.area = new Chart(document.getElementById('chart-area').getContext('2d'),{
    type:'bar',
    data:{labels:items.map(function(x){return x[0];}),
      datasets:[{label:'ครั้ง',data:items.map(function(x){return x[1];}),
        backgroundColor:items.map(function(_,i){return 'hsla('+(185+i*10)+',65%,'+(48+i*3)+'%,0.85)';}),
        borderRadius:6,borderSkipped:false}]},
    options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false},tooltip:{callbacks:{label:function(c){return ' '+c.raw+' ครั้ง';}}}},
      scales:{x:{grid:{color:'rgba(0,0,0,0.04)'},ticks:{font:{size:11}}},y:{grid:{display:false},ticks:{font:{size:11}}}}}
  });
}
